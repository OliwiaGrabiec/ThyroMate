...
const scheduleTabletNotification = async (date) => {
  ...
  const renderItem = ({ item }) => (
    <View style={{  paddingVertical:5 }}>
      <Surface style={styles.surface} elevation={4}>
        ...
        <Button title="Usuń" onPress={() => removeMedicine(item.id)} color="red"/>
      </Surface>
    </View>
  );
  ...
  const addMedicine = async () => {
    ...
    try {
      ...
      console.error('Błąd podczas zapisywania powiadomień w async storage', error);
  ...
  const removeMedicine = async (id) => {
    try {
      ...
      if (medicineToDelete) {
        ...
        setNotifications((prevMedicines) => prevMedicines.filter((medicine) => medicine.id !== id));
      }
    } catch (error) {
  ...
  return (
    <PaperProvider>
      <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
        <ImageBackground source={require('../assets/Leki.png')} style={styles.rootContainer}>
        ...
        <Portal>
            <Dialog visible={dialogVisible} style={{borderRadius:10}} onDismiss={hideDialog}>
              ...
              <Dialog.Actions>
              <Button title="Dodaj" onPress={addMedicine} style={styles.button} color="black"/>
