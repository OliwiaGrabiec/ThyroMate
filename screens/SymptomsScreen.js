...
import { Modal, Portal,  IconButton, Surface, PaperProvider } from 'react-native-paper';
...
const loadSymptomsForDay = async ()  => {
  ...
  try {
    const response = await axios.get(`${BASE_URL}/users/${user_id}/symptoms.json`);
...
const containerStyle = {backgroundColor: 'white', padding: 20};
  const addSymptoms = async () => {
    ...
    setDate(new Date());
    try {
      const response = await axios.post(`${BASE_URL}/users/${user_id}/symptoms.json`, newSymptom);
  ...
  try {
    const symptomToDeleteIndex = symptoms.findIndex(symptom => symptom.id === id);
    if (symptomToDeleteIndex !== -1) {
      ...
      setSymptoms(currentSymptoms => currentSymptoms.filter(symptom => symptom.id !== id));

      setItems(currentItems => {
        const updatedItems = { ...currentItems };
        updatedItems[formattedSymptomDate] = updatedItems[formattedSymptomDate].filter(symptom => symptom.id !== id);
...
const level= ['niski', 'średni', 'wysoki'];
    ...
    <View style={{  paddingVertical:5 }}>
      ...
      <View>//row
        ...
        <Text>{item.description}</Text>
          ...
          alignItems: 'center',
          borderRadius: 30,
          backgroundColor: getBackgroundColor(item.rate,level), // funkcja określająca kolor tła
          }}
          >
        <Text>{item.rate}</Text>
        </View>
        </View>
        <Button title="Usuń" onPress={() => removeSymptoms(item.id)} color="red"/>
      </Surface>
    </View>
  );
  function getBackgroundColor(rate, level) {
    if (rate === level) {
      switch (level) {
        case 'niski': 
          return 'lightblue'; 
            ...
            <Portal>
              <Modal visible={visible} onDismiss={hideModal} contentContainerStyle={containerStyle} >
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-around', padding: 10 }}>
                  ...
                  <TouchableOpacity
                        ...
                        style={{ 
                          ...
                          alignItems: 'center',
                          borderRadius:20, 
                          backgroundColor: getBackgroundColor(rate, level),
                        }}
                ...
                <TextInput
                      ...
                      style={styles.text}
                />
                <Button title="Dodaj" onPress={addSymptoms} style={styles.button} color="pink"/>
                <Button title="Zamknij" onPress={hideModal} style={styles.button} color="pink"/>
              </Modal>
            </Portal>
        </View>
     </PaperProvider>
